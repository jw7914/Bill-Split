...
export async function handleAuthApi(request, response) {
  ...
  if (
    request.method === "GET" &&
    (path === "/api/auth/discord/login" || path.endsWith("/discord/login"))
  ) {
  ...
  if (
    ...
    (path === "/api/auth/discord/callback" ||
      path.endsWith("/discord/callback"))
  ) {
  ...
  if (
    request.method === "GET" &&
    (path === "/api/auth/me" || path.endsWith("/me"))
  ) {
  ...
  if (
    request.method === "POST" &&
    (path === "/api/auth/logout" || path.endsWith("/logout"))
  ) {
...
async function handleDiscordUserLogin(request, response, url) {
  ...
  if (!clientId) {
    sendJson(response, 500, {
      error:
        "DISCORD_CLIENT_ID is missing. Add it from the Discord Developer Portal.",
    });
    return;
  }
...
async function handleDiscordUserCallback(request, response, url) {
  ...
  try {
    ...
    if (!profile?.id || !profile.username) {
      throw new Error(
        "Discord profile response was missing required user fields.",
      );
    }
    ...
    const normalizedGuilds = Array.isArray(guilds)
      ...
      : Array.isArray(existingUser?.guilds)
        ? existingUser.guilds
        : [];
    ...
    const userRecord = {
      ...
      expiresAt: new Date(
        Date.now() + Number(expiresIn || 604800) * 1000,
      ).toISOString(),
...
async function handleGetAuthMe(request, response, url) {
  try {
    ...
    let activeUser = null;
    if (activeUserId) {
      activeUser = await getUser(activeUserId);
    }
